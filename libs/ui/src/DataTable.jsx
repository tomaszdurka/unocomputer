 ...
 * @property {(row: any) => React.ReactNode} cell
 * @property {string} [width] Column width, e.g. '190px' or '30%'. Rendered as a <col>.
 * @property {string} [className] Extra classes on the <td>.
 * @property {string} [headerClassName] Extra classes on the <th>.
 */
 ...
 * @param {(row: any) => string} [props.rowHref] Makes the whole row navigate. The first
 *   column's content is wrapped in a real <Link> so keyboard, middle-click and
 *   open-in-new-tab all work; the rest of the row is a click target on top of that.
 *   Interactive elements in other cells must call event.stopPropagation().
 * @param {React.ReactNode} [props.title] Caption strip above the table.
 * @param {React.ReactNode} [props.headerActions] Right-aligned in the strip. Needs `title`.
 * @param {React.ReactNode} [props.empty] Shown in place of the body when there are no rows.
 * @param {React.ReactNode} [props.footer] Below the table, inside the same bordered box.
 */
