...
export function SelectItem({ className = '', children, ...props }: ItemProps) {
  return (
    <SelectPrimitive.Item
      className={`relative flex cursor-default select-none items-center rounded-lg px-3 py-2 text-sm outline-none data-[disabled]:pointer-events-none data-[highlighted]:bg-gray-100 data-[disabled]:opacity-50 dark:data-[highlighted]:bg-neutral-800 ${className}`}
      {...props}
    >
...
export function SelectLabel({ children }: { children: React.ReactNode }) {
  return (
    <SelectPrimitive.Label className="px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-neutral-500">
      {children}
